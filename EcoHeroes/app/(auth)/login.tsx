...
const ROLES: { value: Role; label: string; icon: string; description: string }[] = [
  { value: 'student', label: 'Student', icon: 'school', description: 'Complete challenges & earn badges' },
  { value: 'teacher', label: 'Teacher', icon: 'people', description: 'Manage a class & approve challenges' },
  { value: 'parent', label: 'Parent', icon: 'heart', description: 'Monitor your child\'s progress' },
]
...
export default function LoginScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (isSignUp && !username.trim()) { showAlert('Missing fields', 'Please enter a username.'); return }
    ...
    return (
      <View style={styles.confirmContainer}>
        <View style={styles.confirmCard}>
          ...
          <Text style={styles.confirmBody}>We sent a confirmation link to:</Text>
          <Text style={styles.confirmEmail}>{confirmedEmail}</Text>
          <Text style={styles.confirmHint}>
            Click the link to activate your account, then come back and sign in.
          </Text>
          <TouchableOpacity style={styles.confirmBtn} onPress={() => {
            setAwaitingConfirmation(false); setIsSignUp(false); setPassword('')
          }}>
          ...
          <TouchableOpacity style={styles.resendBtn} onPress={async () => {
            try {
              const { error } = await (await import('../../lib/supabase')).supabase.auth.resend({ type: 'signup', email: confirmedEmail })
              if (error) throw error
              showAlert('Email resent', 'Check your inbox again.')
            } catch (e: any) { showAlert('Error', e.message || 'Could not resend email.') }
          }}>
    ...
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
            ...
            <View style={styles.roleGrid}>
              {ROLES.map((r) => (
                ...
                >
                  ...
                  />
                  <Text style={[styles.roleCardLabel, role === r.value && styles.roleCardLabelActive]}>
                    {r.label}
                  </Text>
                  <Text style={[styles.roleCardDesc, role === r.value && styles.roleCardDescActive]}>
                    {r.description}
                  </Text>
                </TouchableOpacity>
              ))}
        ...
        {isSignUp && (
          <View style={styles.section}>
            <Text style={styles.label}>
              {role === 'parent' ? 'Your Name' : 'Username'}
            </Text>
            <TextInput
              style={styles.input}
              placeholder={role === 'parent' ? 'Jane Smith' : 'EcoWarrior123'}
        ...
        <View style={styles.section}>
          ...
          {isSignUp && <Text style={styles.passwordHint}>Minimum 6 characters</Text>}
        </View>
...
const styles = StyleSheet.create({
  ...
  roleCard: {
    ...
    alignItems: 'center', borderWidth: 2, borderColor: COLORS.lightGray, gap: 6,
  },
  roleCardActive: { backgroundColor: COLORS.primary, borderColor: COLORS.primary },
  roleCardLabel: { fontSize: 13, fontWeight: '700', color: COLORS.text, textAlign: 'center' },
  roleCardLabelActive: { color: COLORS.white },
  roleCardDesc: { fontSize: 10, color: COLORS.textLight, textAlign: 'center', lineHeight: 14 },
  roleCardDescActive: { color: COLORS.white + 'CC' },
  input: {
  ...
  // Confirmation screen
  confirmContainer: { flex: 1, backgroundColor: COLORS.background, justifyContent: 'center', alignItems: 'center', padding: 24 },
  confirmCard: {
    backgroundColor: COLORS.white, borderRadius: 24, padding: 32, alignItems: 'center',
    width: '100%', maxWidth: 400, shadowColor: '#000', shadowOffset: { width: 0, height: 4 },
    shadowOpacity: 0.1, shadowRadius: 16, elevation: 8,
  },
  confirmIconWrap: { width: 100, height: 100, borderRadius: 50, backgroundColor: COLORS.primary + '15', justifyContent: 'center', alignItems: 'center', marginBottom: 20 },
  ...
  confirmHint: { fontSize: 14, color: COLORS.textLight, textAlign: 'center', lineHeight: 21, marginBottom: 28 },
  confirmBtn: {
